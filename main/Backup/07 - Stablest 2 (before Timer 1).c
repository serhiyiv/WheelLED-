...
void Init_Settings (void)
{
	...
	SETTINGS_IMAGES_CHANGE_TIME = (eeprom_read_byte(5) * 20270UL);
...
//Time in seconts * 1000ms /   Time resolution = number of ticks   3000ms/0.05 = 60000 ticks = 3seconds
if (TicksSleep >=  100000)
{
	EnteringSleep = 1;
	TicksSleep = 0;
}
...
{
	volatile uint32_t TempAngle = AngleIDX;
...
{
	TicksImageTime++;
	if (TicksImageTime>= SETTINGS_IMAGES_CHANGE_TIME) // Time in seconts * 1000ms /   Time resolution = number of ticks   3000ms/0.05 = 60000 ticks = 3seconds
	{
		NextImage = 1;
		TicksImageTime =0;
...
cli();
	...
	{
		NextImage = 0;
		volatile uint32_t TempImageIDX = ImageIDX;		
