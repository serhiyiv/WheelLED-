...
volatile uint32_t IMAGE_SIZE = 12288;
...
volatile  uint32_t  Ticks = 50000;
volatile  uint32_t  TicksTemp = 0;
volatile  uint32_t  TicksSleep = 0;
volatile  uint32_t  TicksImageTime = 0;
...
volatile uint32_t   AngleCount = 360;
volatile uint32_t  AngleTime = 200;
...
volatile uint32_t LastAngle = -1;
volatile uint32_t ImageIDX = 0;
volatile uint32_t CurrentImageAddress = 0;
...
void Init_Settings (void)
{
	...
	SETTINGS_ANGLE_OFFSET       = eeprom_read_byte(1) * 2; 
	...
	SETTINGS_IMAGES_CHANGE_TIME = (eeprom_read_byte(5) * 46875UL);
	SETTINGS_SLEEP_TIME = (eeprom_read_byte(6) * 46875UL);
	
	
//SETTINGS_IMAGES_CHANGE_TIME = 3UL * 20000UL;
...
USB_PUBLIC uchar usbFunctionSetup(uchar data[8]) {
    ...
    case USB_DATA_OUT: // send data to PC
        replyBufint[0] = 121;
	    usbMsgPtr = replyBufint;
        return sizeof(replyBufint);
         
		 
    case READ_FLASH: // send data to PC
       	 replyBufint[0] = sst25vf_ReadByte(MEMORY_POS);
	     MEMORY_POS++;
	     usbMsgPtr = replyBufint;
	     return sizeof(replyBufint);	 
		
    case READ_EEPROM: // send data to PC
		replyBufint[0] = eeprom_read_byte(SETTINGS_POS);
...
{
	sst25vf_Read_Reverse(ImageIDX * IMAGE_SIZE + AngleIDX_wOffset * 24, l74hc595_icarray, 24); 
	l74hc595_shiftout();
} 
else if (SETTINGS_DISPLAY_MODE == 1)// 1 - Different Images on Different Sides
{
	volatile uint32_t   NextImageIDX ;
	volatile uint32_t  TempNextImageIDX = ImageIDX + 1;
	...
	for (int a = 0; a < 12; a++)
	{
		l74hc595_setchipbyte(a, sst25vf_ReadByte(ImageIDX * IMAGE_SIZE + AngleIDX_wOffset * 24 + a));
		l74hc595_setchipbyte(12 + a, sst25vf_ReadByte(NextImageIDX * IMAGE_SIZE + AngleIDX_wOffset * 24 + a + 12));
	}
	l74hc595_shiftout();	
}
...
else if (SETTINGS_DISPLAY_MODE == 2 ||SETTINGS_DISPLAY_MODE == 3) // 2 - Front only. Rear OFF  // 3 - Rear Only. Front OFF
{
  l74hc595_setregallon(0);
	for (int a = 0; a < 12; a++)
	{
		if (SETTINGS_DISPLAY_MODE == 2) { l74hc595_setchipbyte(a, sst25vf_ReadByte(ImageIDX * IMAGE_SIZE + AngleIDX_wOffset * 24 + a));}
		if (SETTINGS_DISPLAY_MODE == 3) { l74hc595_setchipbyte(12 + a, sst25vf_ReadByte(ImageIDX * IMAGE_SIZE + AngleIDX_wOffset * 24 + a + 12));}
	}
	l74hc595_shiftout();
...
else if (SETTINGS_DISPLAY_MODE == 4) // 4 - Toggle Sides. One side always Off
{
	...
	for (int a = 0; a < 12; a++)
	{
		if (DisplaySide == SIDE_LEFT) { l74hc595_setchipbyte(a, sst25vf_ReadByte(ImageIDX * IMAGE_SIZE + AngleIDX_wOffset * 24 + a));}
		if (DisplaySide == SIDE_RIGHT) { l74hc595_setchipbyte(12 + a, sst25vf_ReadByte(ImageIDX * IMAGE_SIZE + AngleIDX_wOffset * 24 + a + 12));}
	}
	l74hc595_shiftout();	
}
...
void ProcessButtons(Void)
{
	    ...
	    {
		    Delay_ms(100);
		    ButtonRes = Button_Pressed(Button_Pin1);
		    if ( ButtonRes == 1)
		    {
		    ...
		    {
			    l74hc595_setregallon(0);
			    l74hc595_setregoff((ImageIDX + 1) * 3 - 1);
			    l74hc595_shiftout();
			    ImageIDX++;
		    }
	     ...
	     ButtonRes = Button_Pressed(Button_Pin2);
	    if ( ButtonRes == 1)
	    {
		    Delay_ms(100);
		    ButtonRes = Button_Pressed(Button_Pin2);
